...
def implode_fattened_df(df):
    ...
    index = pd.MultiIndex.from_tuples(tuples, names=['Section', 'Subsection','Field'])
    ...
    for fill_name,fill_item in zip(df_grouped.index,df_grouped.values):
        ...
        elif fill_item is not None and fill_item !=np.nan and str(fill_item)!='nan':
            continue
        else:
            #if not none or nan and not a list make a list
            fill_item=[fill_item]
            
        current_values=fill_.loc[fill_name.split('||')[1],fill_name.split('||')[2],fill_name.split('||')[3]][fill_name.split('||')[0]]
        if current_values!=np.nan and str(current_values)!='nan' and current_values is not None:
        ...
        for value_new in current_values:
            if value_new is not None and value_new !=np.nan and str(value_new)!='nan':
                if isinstance(value_new, list):
                    for value_new_ in value_new:
                        if value_new_ is not None and value_new_ !=np.nan and str(value_new_)!='nan':
        ...
        fill_.loc[fill_name.split('||')[1],fill_name.split('||')[2],fill_name.split('||')[3]][fill_name.split('||')[0]]=current_values_clean 
    ...
    for section_,sub_section_lists in results.items():
        ...
        if isinstance(sub_section_lists, dict):
            for sub_section,sub_section_list in sub_section_lists.items():
                if sub_section=='straininfo_link':
                    continue 
        ...
        #refs  
        elif isinstance(sub_section_lists, list):
