...
class TestClean(unittest.TestCase):
    ...
    def get_flatten_df_and_implode_fattened_df(self):

        ulr_test='https://bacdive.dsmz.de/api/bacdive/bacdive_id/132558/?format=json'
        test_json=get_data_path('test.json')
        test_df=pd.read_csv(get_data_path('test.csv'), index_col=[0,1,2]).sort_index()
        ...
        pd.testing.assert_frame_equal(truth_df,test_data)
